...
export class AIDocumentProcessor {
  ...
  constructor(
    ...
    this.apiKey = apiKey || import.meta.env.VITE_AI_API_KEY || '';
    this.apiBaseUrl = apiBaseUrl || import.meta.env.VITE_API_URL || 'http://localhost:8000';
  }
  ...
  async processDocument(file: File): Promise<ExtractedPropertyData> {
    try {
      if (this.provider === 'openai') {
        return await this.processViaOpenAI(file);
      } else if (this.provider === 'google') {
        return await this.processViaGoogleVision(file);
      } else if (this.provider === 'aws') {
        return await this.processViaAwsTextract(file);
      }
      
      // Default / MVP: send to backend
